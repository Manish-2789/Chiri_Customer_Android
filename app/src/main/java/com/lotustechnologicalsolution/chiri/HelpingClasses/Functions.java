...
//    get marker pin with rotation
    ...
    public static Bitmap createDrawableMarker(Activity activity) {
        View markerLayout = activity.getLayoutInflater().inflate(R.layout.custom_map_marker, null);

        /*LayerDrawable layerDrawable = (LayerDrawable) ((RelativeLayout)markerLayout.findViewById(R.id.rl_background)).getBackground();
        GradientDrawable outerCircle = (GradientDrawable) layerDrawable.findDrawableByLayerId(R.id.circle_outer);

        String outerColor = "#44"+colorCode.trim();
        outerCircle.setColor(Color.parseColor(outerColor));
        GradientDrawable innerCircle = (GradientDrawable) layerDrawable.findDrawableByLayerId(R.id.circle_inner);

        String innerColor = "#"+colorCode.trim();
        innerCircle.setColor(Color.parseColor(innerColor));*/


        //TextView tvOrderNo = (TextView) markerLayout.findViewById(R.id.tv_orderno);
